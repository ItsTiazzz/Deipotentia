...
public class HephaestusForgeBlock extends BlockWithEntity implements BlockEntityProvider {
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (state.getBlock() != newState.getBlock()) {
            ...
            if (blockEntity instanceof HephaestusForgeBlockEntity) {
                ItemScatterer.spawn(world, pos, (HephaestusForgeBlockEntity)blockEntity);
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(type, DBlockEntities.HEPHAESTUS_FORGE, HephaestusForgeBlockEntity::tick);
    }
}
